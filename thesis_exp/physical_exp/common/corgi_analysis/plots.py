...
def plot_position_error(ekf: dict, err_3d: np.ndarray, results_dir: str,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'fig03_position_error.pdf'))
    plt.close()
...
def plot_attitude(ekf: dict, vicon_i: dict, ekf_rpy: np.ndarray,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'fig05_attitude_rpy.pdf'))
    plt.close()


def plot_bias(ba: dict, bw: dict, results_dir: str, T_WALK_END: float):
    """fig06_accel_bias.pdf, fig07_gyro_bias.pdf"""
    for fname, data, ylabel, title in [
        ('fig06_accel_bias.pdf', ba, 'Accel bias [m/s²]', 'Accelerometer Bias ba'),
        ('fig07_gyro_bias.pdf',  bw, 'Gyro bias [rad/s]', 'Gyroscope Bias bw'),
    ]:
        fig, ax = plt.subplots(figsize=(12, 3))
...
def plot_foot_heights(vi, results_dir: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'fig08_foot_heights.pdf'))
    plt.close()


def plot_contact_timeline(vi, gmo: dict, results_dir: str,
                           T_WALK_END: float, label: str = 'EKF'):
    """fig09_contact_timeline.pdf"""
    ...
    axes[-1].set_xlabel('Time [s]')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'fig09_contact_timeline.pdf'))
    plt.close()
