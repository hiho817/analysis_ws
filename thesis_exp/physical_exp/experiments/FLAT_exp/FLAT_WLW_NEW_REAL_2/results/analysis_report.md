...
|----------|------|--------------|--------|--------|--------|---------|--------|--------|--------|---------|
| FLAT_WLW_NEW_REAL_2 | NEW_WLW | 0.0–34.9 | 4.81 | 0.59 | 1.41 | 5.05 | 0.019 | 0.026 | 0.012 | 0.034 |
...
| WLW (輪足步行) | ESEKF | 1 | 4.811 ± 0.000 | 0.592 ± 0.000 | 1.408 ± 0.000 | 5.048 ± 0.000 | 0.019 ± 0.000 | 0.026 ± 0.000 | 0.012 ± 0.000 | 0.034 ± 0.000 |
| WLW (輪足步行) | Legacy | 0 | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan |
| MPC (模型預測控制) | ESEKF | 0 | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan | nan ± nan |
...
| WLW | 5.05 ± 0.00 | nan ± nan | +nan% |
| MPC | nan ± nan | nan ± nan | +nan% |
...
| 實驗編號 | Roll (°) | Pitch (°) | Yaw (°) |
|----------|----------|-----------|---------|
| FLAT_WLW_NEW_REAL_2 | 0.461 | 0.332 | 0.377 |
...
|----------|-------------|-------------|--------------|
| FLAT_WLW_NEW_REAL_2 | 1.37 | 1.04 | 1.72 |

---

## 5. 接觸偵測指標（逐有效時間步比對）

僅使用 VICON 腳標記有效、且 GMO 實際有資料的重疊時間步。VICON 接觸真值由 Ground1–Ground4 擬合的無限地面平面計算，腳標記高度低於 **20 mm** 視為接觸。GMO 使用本試驗以 VICON 校正的 OR-Schmitt 門檻：`F_rm` 進／離地為 **50/25**，`tau_beta` 進／離地為 **3.00/1.50**。在每個有效時間步直接比對 VICON 與 GMO 的二元接觸狀態，Acc = (TP + TN) / N。不進行接觸或離地事件配對，也不計算延遲。四腳平均為各腳 accuracy 的算術平均。
...
|------|--------------|-------------------|--------------------------|--------------------------|
| ESEKF (NEW) | N/A m | N/A m | N/A | N/A cm |
| Legacy (OLD) | N/A m | N/A m | N/A | N/A cm |

**分析：**
- ESEKF（NEW）：估測器回報 N/A m，VICON 實際停止 N/A m，平均停止誤差 **N/A cm**（< 2 cm）。估測結果與實際高度吻合，控制器能精準停止於目標位置。
...
- **ESEKF**：VICON 實際停止 N/A m，誤差 **N/A cm** ✓
