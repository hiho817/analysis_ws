...
from corgi_analysis.bag_loader   import load_fusion_bag, load_inner_ekf_bag

# ─── Paths ─────────────────────────────────────────────────────────────────────
BASE      = os.path.dirname(os.path.abspath(__file__))
BAG_FUSION = os.path.join(BASE, '..', 'bag',
...
T_END = min(vi.t_trigger_end,
            fbag['t_trigger_end'],
            abag['t_trigger_end'])
print(f'Common window: t ∈ [0, {T_END:.2f}] s')
...
for ai, ax in enumerate(axes):
    ax.plot(t_vi[vi_ok],           pos_vi[vi_ok, ai],  lw=LW+0.4, color=C_VI,   label='VICON (GT)',       zorder=4)
    ax.plot(fe_t,                  [fe_px, fe_py, fe_pz][ai],  lw=LW, color=C_EKF,  label='Inner EKF (fusion bag)',  zorder=3)
    ax.plot(om_t,                  [om_px, om_py, om_pz][ai],  lw=LW, color=C_ODOM, label='odom_mapping',   zorder=2, ls='--')
    ax.plot(ae_t,                  [ae_px, ae_py, ae_pz][ai],  lw=LW, color=C_ABL,  label='Ablation EKF',  zorder=1, ls=':')
...
for ai, ax in enumerate(axes):
    # VICON velocity (body frame, noisy near edges)
...
# ══════════════════════════════════════════════════════════════════════════════
# Figure 3: fusion/bv 補正量說明圖
#   上：bv_x  bv_y  bv_z 時序
#   中：|bv| magnitude vs 累積 leg-odom 位置誤差（ablation - VICON）
#   下：說明累積效應
# ══════════════════════════════════════════════════════════════════════════════
...
if len(fv_t) > 1:
    ...
    cum_bv_correction = np.cumsum(fv_mag * dt_fv)   # [m]  ≈ total correction applied
else:
...
ax.set_title('fusion/bv velocity bias correction  (mean |bv| ≈ 7.2 mm/s)', fontsize=9)

# Panel 2: cumulative bv correction vs actual position error reduction
ax = axes[1]
# Position error for ablation and full-fusion
...
ax = axes[2]
ax.plot(ae_t, abl_err_y*100, color=C_ABL, lw=LW, ls=':', label='Ablation Y 誤差 [cm]')
ax.plot(fe_t, fek_err_y*100, color=C_EKF, lw=LW,        label='Full fusion Y 誤差 [cm]')
# Cumulative bv_y correction
if len(fv_t) > 1:
