#!/usr/bin/env python3
"""
CORGI Experiment Analysis — 20260522 exp1
mpc_esekf_gmo (ESEKF + GMO contact_state)

Steps: Contact, Inner EKF (pos/vel/att), Outer Fusion, LiDAR quality
...
BAG_DB    = os.path.join(BASE, '..', 'bags',
                         'mpc_esekf_20260522_193236',
                         'mpc_esekf_20260522_193236_0.db3')
VICON_CSV = os.path.join(BASE, '..', 'vicon', 'EXP2.csv')
TRIAL     = 'mpc_esekf_gmo'
DATE      = '20260522'
EXP_ID    = 'exp2'
...
# Align VICON initial orientation to EKF
...
if vl.sum() >= 3:
    ...
    T_CO_rpy = rpy_CO; T_CO_t = t_CO
    resid_mean = resid.mean() * 100; resid_max = resid.max() * 100
else:
    print('  [WARN] not enough LiDAR points')
...
fig.suptitle(f'Contact Detection (green=VICON, orange=GMO) — {DATE} {EXP_ID}')
fig.tight_layout()
...
z_offset_z = epz[0] - vi_pz_e[0]   # ~0.20 m body height vs VICON origin
err_px = epx - vi_px_e; err_py = epy - vi_py_e; err_pz = epz - vi_pz_e - z_offset_z
...
print(f'  RPY RMSE: roll={metrics_rpy["RMSE_roll_deg"]:.2f}° '
      f'pitch={metrics_rpy["RMSE_pitch_deg"]:.2f}° yaw={metrics_rpy["RMSE_yaw_deg"]:.2f}°')
...
ax.plot(epx[0], epy[0], 'go', ms=8, label='EKF start')
ax.plot(epx[-1], epy[-1], 'r^', ms=8, label='EKF end')
plt.colorbar(sc, ax=ax, label='Time [s]')
ax.set_xlabel('X [m]'); ax.set_ylabel('Y [m]')
ax.set_title(f'EKF XY Trajectory — {DATE} {EXP_ID}')
ax.set_aspect('equal'); ax.legend(fontsize=8); ax.grid(True, alpha=0.4)
fig.tight_layout()
fig.savefig(os.path.join(RESULTS, 'fig_ekf_xy.png'), dpi=150); plt.close(fig)

# Position time series
fig, axes = plt.subplots(3, 1, figsize=(11, 8), sharex=True)
_ekf_plot = {'px': epx, 'py': epy, 'pz': epz - z_offset_z}
for ax, key, lbl, vi_val in zip(axes, ['px','py','pz'], ['X','Y','Z'],
                                 [vi_px_e, vi_py_e, vi_pz_e]):
    cov = ekf[f'cov_p{lbl.lower()}'][ekf_mask]
    sigma = np.sqrt(np.abs(cov))
    ep_plot = _ekf_plot[key]
    ax.fill_between(et, ep_plot-3*sigma, ep_plot+3*sigma, alpha=0.2, label='3σ')
    ax.plot(et, ep_plot, lw=0.8, label='EKF')
...
fig.tight_layout()
fig.savefig(os.path.join(RESULTS, 'fig_ekf_pos.png'), dpi=150); plt.close(fig)

# Velocity
fig, axes = plt.subplots(3, 1, figsize=(11, 8), sharex=True)
for ax, ek_val, vi_val, lbl in zip(axes, [evx, evy, evz],
                                    [vi_vx_e, vi_vy_e, vi_vz_e], ['vx','vy','vz']):
...
axes[-1].set_xlabel('Time [s]')
fig.suptitle(f'EKF Velocity — {DATE} {EXP_ID}')
fig.tight_layout()
fig.savefig(os.path.join(RESULTS, 'fig_ekf_vel.png'), dpi=150); plt.close(fig)

# RPY
fig, axes = plt.subplots(3, 1, figsize=(11, 8), sharex=True)
for ax, ek_val, vi_val, lbl in zip(axes,
                                    [rpy_ekf_deg[:,0], rpy_ekf_deg[:,1], rpy_ekf_deg[:,2]],
                                    [vi_roll_e, vi_pitch_e, vi_yaw_e], ['Roll','Pitch','Yaw']):
    ax.plot(et, ek_val, lw=0.8, label='EKF')
    valid_vi = ~np.isnan(vi_val)
    ax.plot(et[valid_vi], vi_val[valid_vi], 'k--', lw=1, alpha=0.7, label='VICON')
    _shade_window(ax, T_END, label=False)
...
axes[-1].set_xlabel('Time [s]')
fig.suptitle(f'EKF Attitude — {DATE} {EXP_ID}')
fig.tight_layout()
fig.savefig(os.path.join(RESULTS, 'fig_ekf_rpy.png'), dpi=150); plt.close(fig)
...
metrics_odom_pos = {
    ...
    'final_odom_x': float(opx[-1]), 'final_odom_y': float(opy[-1]),
}
...
print(f'  odom_mapping 2D RMSE: {metrics_odom_pos["RMSE_2D_vs_VICON_cm"]:.2f}cm  '
      f'yaw RMSE: {metrics_odom_yaw["RMSE_yaw_vs_VICON_deg"]:.2f}°')

fv_mask = (fv['t'] >= 0.0) & (fv['t'] <= T_END)
fv_t = fv['t'][fv_mask]; fv_x = fv['x'][fv_mask]; fv_y = fv['y'][fv_mask]
fv_mag = np.sqrt(fv_x**2 + fv_y**2)
metrics_fv = {
    'mean_bv_x': float(np.nanmean(fv_x)), 'mean_bv_y': float(np.nanmean(fv_y)),
    'mean_bv_mag': float(np.nanmean(fv_mag)), 'max_bv_mag': float(np.nanmax(fv_mag)),
}
...
ax.set_aspect('equal'); ax.legend(fontsize=8); ax.grid(True, alpha=0.4)
fig.tight_layout()
fig.savefig(os.path.join(RESULTS, 'fig_fusion_xy.png'), dpi=150); plt.close(fig)
...
lpx_o = lidar['px_odom'][lidar_mask]; lpy_o = lidar['py_odom'][lidar_mask]
lidar_rate = len(lt) / (lt[-1] - lt[0]) if len(lt) > 1 and (lt[-1]-lt[0]) > 0 else float('nan')
dp = np.sqrt(np.diff(lpx_o)**2 + np.diff(lpy_o)**2) if len(lpx_o) > 1 else np.array([])
jumps = int(np.sum(dp > 0.1))
metrics_lidar = {'n_msgs': int(lidar_mask.sum()), 'rate_hz': float(lidar_rate), 'n_jumps_gt10cm': jumps}
...
ax.set_aspect('equal'); ax.legend(fontsize=8); ax.grid(True, alpha=0.4)
fig.tight_layout()
fig.savefig(os.path.join(RESULTS, 'fig_lidar_xy.png'), dpi=150); plt.close(fig)
...
all_metrics = {
    ...
    'lidar': metrics_lidar,
    'T_CO': {'RPY_deg': list(T_CO_rpy), 't_m': list(T_CO_t),
...
print(f'  EKF 3D RMSE:  {metrics_pos["RMSE_3D_cm"]:.2f} cm')
print(f'  Yaw RMSE:     {metrics_rpy["RMSE_yaw_deg"]:.2f}°')
